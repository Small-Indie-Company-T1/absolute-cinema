...
from .serializers import AddWatchlistSerializer, RemoveWatchlistSerializer, WatchlistSerializer
...
class WatchlistViewSet(viewsets.ModelViewSet):
    ...
    def remove_from_watchlist(self, request):
        ...
        return Response({"status":"success", "message": "Фильм удален из списка просмотра"}, status=status.HTTP_200_OK)
    ...
    def mark_as_watched(self, request):
        ...
        return Response({"status":"success", "message": "Фильм отмечен как просмотренный"}, status=status.HTTP_200_OK)